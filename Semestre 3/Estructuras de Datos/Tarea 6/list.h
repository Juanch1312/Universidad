...
template <class T, int ARRAYSIZE = 4096>
class List{
private:
    T data[ARRAYSIZE];
    int last;
...
public:
    ...
    void sortDataBubble(const int&);
    void sortDataInsert(const int&);
    void sortDataSelect(const int&);
    void sortDataShell(const int&);
...
template< class T, int ARRAYSIZE>
void List<T,ARRAYSIZE>::sortDataBubble(const int& aux) ///Burbuja mejorada
{
    int i(last), j;
    bool flag;
if(aux==1){
    ...
    do{
        ...
        while(j<i)
        {
            if(data[j].GetNameSong()>data[j+1].GetNameSong())
            {
                swapData(data[j],data[j+1]); ///intercambio
...
template< class T, int ARRAYSIZE>
void List<T,ARRAYSIZE>::sortDataInsert(const int& nt) ///Insert
{
  int i(1),j;
  T aux;
  if(nt==1){
       while(i<= last)
       {
           aux.GetAuthor() = data[i].GetAuthor();
           ...
           if(i!=j){
            data[j].GetAuthor() = aux.GetAuthor();
           }
           i++;
       }
     }
       ...
       while(i<= last)
       {
           aux.GetNameSong() = data[i].GetNameSong();

           j=i;
           while(j > 0 and aux.GetNameSong() < data[j-1].GetNameSong())
           {
               swapData(data[j],data[j-1]);
           ...
           if(i!=j){
            data[j].GetNameSong()= aux.GetNameSong();
           }
           i++;
       }
     }
...
template< class T, int ARRAYSIZE>
void List<T,ARRAYSIZE>::sortDataSelect(const int& aux) ///Select
{
  if(aux==1){
       int i(0),j,m;
       ...
       while(i<last){
         ...
         j=i+1;
         while(j<=last){
           if(data[j].GetNameSong()<data[m].GetNameSong()){
             m=j;
           }
...
template< class T, int ARRAYSIZE>
void List<T,ARRAYSIZE>::sortDataShell(const int& aux) ///Shell
{
  if(aux==1){
       ...
       while(dif>0){
         ...
         while(i<=last){
           j=i;
           while(j>=dif and data[j-dif].GetNameSong()>data[j].GetNameSong()){
             swapData(data[j-dif],data[j]);
